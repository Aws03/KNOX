...
namespace JadaraITKnowledgeSystem.Domain.Quizzes.Entites
{
    public sealed class Choice : AuditableEntity
    {
        ...
        private Choice(int questionId, string text, bool isCorrect)
        {
        ...
        public static Result<Choice> Create(int questionId, string text, bool isCorrect)
        {
            if (string.IsNullOrWhiteSpace(text))
            {
                return QuizErrors.ChoiceTextRequired;
            }
            var choice = new Choice(questionId, text, isCorrect);
            return choice;
        }
    }
